...
def sol(T, N):
    ...
    for i in range(1, N):
        current = Counter(T[i])
        diff = sum((target - current).values()) + sum((current - target).values())  # 두 Counter 간의 차이를 계산

        if len(T[i]) == len(T[0]):
            if diff <= 2:  # 길이가 같을 때, 차이가 2 이하면 유사함
                result += 1
        elif abs(len(T[i]) - len(T[0])) == 1:
            if diff <= 1:  # 길이 차이가 1일 때, 차이가 1 이하면 유사함
                result += 1
